function newDepth = computeGuidedDepthKeyboard(d,g)
    %% guidance image
    d1 = g;
    ...
    radius=4;
    d13=WeightedGuidedImageFilter(d, g, radius, 0.2);
...
%     d8=d8.*max(max(d));
    ...
    %     lambda = 0.1;           % smoothness parameter for synthetic
    % lambda = 3;           % smoothness parameter for synthetic
    % lambda = 2;           % smoothness parameter for synthetic
    lambda = 900;           % for real
    mu = 100;              % bw for static guidance
    nu = 50;              % bw for dynamic guidance
    step=10;
    issparse = true;
