function newDepth = computeGuidedDepthSimc(d,g)
    %% guidance image
    d1 = g;
...
%     d10 = AD(d,g,10,0.1);
    ...
    d12=jointWMF(d,g,2,5,256,256,1,'exp');       % WMF (100+ times faster weighted median fitler)
    
    %%  Weighted Guided Image Filtering
    radius=2;
    d13=WeightedGuidedImageFilter(d, g, radius, 0.10, 1);
    
    %     %% Rolling Guidance Filter
    %     d6 = RollingGuidanceFilter(d,3,0.1,10); % far better than below
...
%     d8=d8.*max(max(d));
    ...
    lambda_G = 20;    % for guidance image
    maxiter = 20;
    %     r = 2; % it was bad
    r = 2;
    ...
    u0=ones(m,n);
    nei= 1; % better than 0
    lambda = 1;           % smoothness parameter for synthetic
    mu = 20;              % bw for static guidance
    nu = 10;              % bw for dynamic guidance
    step=10;
    issparse = true;
